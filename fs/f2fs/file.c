				     ...
				     unsigned end, struct fi_merge_manage *fmm, u32 max_blocks)
{
	...
	trace_printk("count=%u index=%u length=%u\n",
				 ...
				 struct file_list_node *fm_node, struct merge_extent *fm_ext)
{
	...
	trace_printk("f2fs_insert_merge_extebt low=%u high=%u\n", low, high);
	for (i = low; i <= high; i++) {
...
static int end_file_merge(struct file *filp, unsigned long arg)
{
	...
	trace_printk("f2fs_ioc_end_file_merge fill summary num=%d,cur_blocks=%lu\n",
	...
	trace_printk("f2fs_ioc_end_file_merge write summary size:%lu. offset:%lld tail:%d, checksum:%u\n",
	...
	ret = merge_sync_file(sbi, filp);
fail:
	trace_printk("f2fs_ioc_end_file_merge ret:%d, size:%lld\n", ret, inode->i_size);
	...
	for (i = 0; i < summary->num; i++) {
		...
		for (j = 0; j < merge_file.extent_count; j++) {
			...
			for (k = 0; k < extent.length; k++, pos += PAGE_SIZE) {
				...
				trace_printk("f2fs_ioc_preload_file preload ino = %u, index_in_mfile = %u, index = %lu, pos = %llu, checksum = %u\n",
